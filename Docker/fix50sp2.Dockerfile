...
# FIX 5.0 SP2 (FIXT.1.1 transport) server image.
#
# Build from the repository root so the context includes the whole workspace:
#   docker build -f Docker/fix50sp2.Dockerfile -t ironfix-fix50sp2:latest .
#
...
COPY --from=builder /fix50sp2_server /app/fix50sp2_server
